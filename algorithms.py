...
def eaSimple_mod(population, toolbox, cxpb, mutpb, ngen, state, player_id, attrAction, stats=None,
             halloffame=None, verbose=__debug__):
    """This algorithm reproduce the simplest evolutionary algorithm as
    presented in chapter 7 of [Back2000]_.
    ...
    # Evaluate the individuals with an invalid fitness
    invalid_ind = [ind for ind in population if not ind.fitness.valid]
    fitnesses = toolbox.map(toolbox.evaluate, invalid_ind,[state]*len(invalid_ind),\
                            [player_id]*len(invalid_ind),[attrAction]*len(invalid_ind))
